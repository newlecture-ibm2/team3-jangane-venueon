package com.venueon.host.session.adapter.in.web;
...
/**
 * Host 전용 세션 API.
 * 기존 event/presentation/HostSessionController를 host/session/adapter/in/web/ 표준 위치로 이동.
 */
@RestController
...
public class HostSessionController {
    ...
    public ApiResponse<SessionResponse> createSession(
        ...
        var command = new CreateSessionUseCase.CreateSessionCommand(
                ...
                request.title(), request.description(), request.sortOrder(),
                request.startTime(), request.endTime(),
                request.location(), request.regionSido(), request.regionSigungu(),
                request.addressRoad(), request.addressDetail(),
                request.isOnline(), request.onlineLink(),
                request.maxAttendees(),
                request.recruitStartDate(), request.recruitEndDate()
        );
    ...
    public ApiResponse<SessionResponse> updateSession(
        ...
        var command = new UpdateSessionUseCase.UpdateSessionCommand(
                sessionId, eventId, hostId, userRole,
                request.title(), request.description(), request.sortOrder(),
                request.startTime(), request.endTime(),
                request.location(), request.regionSido(), request.regionSigungu(),
                request.addressRoad(), request.addressDetail(),
