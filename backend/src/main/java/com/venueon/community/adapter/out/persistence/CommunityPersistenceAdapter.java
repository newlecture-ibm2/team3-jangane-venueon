...
public class CommunityPersistenceAdapter implements CommunityRepositoryPort {
    ...
    @Override
    public Page<Community> findJoinedCommunities(java.util.List<Long> memberCommunityIds, java.util.List<Long> orderEventIds, java.util.List<Long> badgeEventIds, Pageable pageable) {
        if ((memberCommunityIds == null || memberCommunityIds.isEmpty()) && 
            (orderEventIds == null || orderEventIds.isEmpty()) && 
            (badgeEventIds == null || badgeEventIds.isEmpty())) {
            return Page.empty(pageable);
        }
        
        if (memberCommunityIds == null) memberCommunityIds = new java.util.ArrayList<>();
        if (orderEventIds == null) orderEventIds = new java.util.ArrayList<>();
        if (badgeEventIds == null) badgeEventIds = new java.util.ArrayList<>();
        
        if (memberCommunityIds.isEmpty()) memberCommunityIds = java.util.List.of(0L);
        if (orderEventIds.isEmpty()) orderEventIds = java.util.List.of(0L);
        if (badgeEventIds.isEmpty()) badgeEventIds = java.util.List.of(0L);

        return communityJpaRepository.findJoinedCommunities(memberCommunityIds, orderEventIds, badgeEventIds, pageable)
                .map(this::mapToDomain);
    }
