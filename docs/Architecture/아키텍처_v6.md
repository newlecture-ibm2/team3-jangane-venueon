...
| **com.venueon.event** | 이벤트 CRUD, Session 관리 (일정/장소/정원), 상태 Computed (EventStatus + RecruitmentStatus), 리뷰 |
| **com.venueon.ticket** | Ticket CRUD, 가격/할인, 판매 기간/수량 관리, 세션 매핑 |
...
| **com.venueon.admin** | 관리자 대시보드, 회원 정지/권한, 카테고리 관리, 리뷰 관리, 커뮤니티 제재 |
...
| **com.venueon.notice** | 통합 공지, 요청 게시판, 이의 제기 게시판 |
...
> [!IMPORTANT]
> 날짜 필드는 모두 nullable. 미설정 시 자동 전이 없음 → 호스트 수동 관리.
