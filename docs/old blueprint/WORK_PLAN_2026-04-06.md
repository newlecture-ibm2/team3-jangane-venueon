...
graph LR
    ...
    P2 --> P3[Phase 3<br/>이벤트 생성]
    P3 --> P4[Phase 4<br/>이벤트 수정]
```
...
### Phase 4. ✏️ 이벤트 수정 (edit)
**예상:** ~30분 | **BE + BFF + FE 풀스택**

Phase 3의 EventForm을 재사용하므로 가장 빠릅니다.
...
| 3 | 이벤트 생성 | BE+BFF+FE | 1시간 20분 | 3시간 |
| 4 | 이벤트 수정 | BE+BFF+FE | 30분 | **3시간 30분** |
...
Phase 2 (상세) → 목록의 카드 클릭으로 진입, Phase 1의 인프라 재사용
    ↓
Phase 3 (생성) → Host 전용, 쓰기 API는 읽기와 분리하여 나중에
    ↓
Phase 4 (수정) → Phase 3의 EventForm 재사용, 가장 적은 공수
```
...
- [ ] Phase 3: 이벤트 생성 (new) — BE+BFF+FE
- [ ] Phase 4: 이벤트 수정 (edit) — BE+BFF+FE
