...
| **어드민 고도화** | 🟡 기본 대시보드 | 🆕 회원 정지/권한 + 카테고리 관리 + 관심 카테고리(!카테고리 관리에 포함) + 커뮤니티 제재 |
...
| 14 | **어드민 대시보드** | 회원 정지/권한 + 카테고리 + 관심 카테고리 (!관심카테고리는 수강생이 관심있는 카테고리)+ 제재 관리 | ✅ 기본 → 🆕 대폭 고도화 |
| 15 | **뱃지 시스템** | 자동 발급 + 노출 설정 + 보유자 검색/초대 (!초대장 못보냄) + 커뮤니티 매칭 | 🆕 신규 |
| 16 | **사용자 공개 프로필** | 보유 뱃지 + 활동 내역 (글/댓글/리뷰/커뮤니티)(!이걸 다 공개해도될까요) (+!커리어 이력 프로필에 포함)| 🆕 신규 |
...
| **환불** | USER 환불 요청, HOST 직접 환불, ADMIN 승인/거절(!환불 지연 건 재촉만 가능) | 🆕 확장 |
